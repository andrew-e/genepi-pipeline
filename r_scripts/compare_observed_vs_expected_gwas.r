...
clumped_filenames <- split_string_into_list(args.clumped_filenames)

if (length(gwas_filenames) != length(clumped_filenames)) {
  stop("List size of --gwas_filenames and --clumped_filenames are different.  They must be the same, and corresponding.")
}
